...
- (UIColor *)colorByLuminanceMapping {
    NSAssert(self.canProvideRGBComponents, @"Must be an RGB color to use arithmatic operations");
    ...
    return [UIColor colorWithWhite:(r * 0.2126 + g * 0.7152 + b * 0.0722) alpha:a];
}
...
- (UIColor *)colorByMultiplyingByColor:(UIColor *)color {
    ...
    CGFloat r, g, b, a;
    if (![color red:&r green:&g blue:&b alpha:&a]) {
        return nil;
    }
...
- (BOOL)isDark {
    CGFloat gray = self.red * 0.299 + self.green * 0.587 + self.blue + 0.114;   //纯色为白，纯黑为0
...
#pragma mark UIColor Expanded initialization

+ (void)load {
    colorNameCache = [NSMutableDictionary dictionary];
}
...
#if SUPPORT_UNDOCUMENTED_API
@implementation UIColor (UnDocumented_Common)
