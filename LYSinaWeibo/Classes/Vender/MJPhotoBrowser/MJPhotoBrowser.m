...
@interface MJPhotoBrowser () <MJPhotoViewDelegate, MJPhotoToolBarDelegate, LYActionSheetDelegate>
{
    // 滚动的view
...
- (void)loadView
{
    ...
    UILongPressGestureRecognizer *pressGes = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(showActionSheet:)];
    [self.view addGestureRecognizer:pressGes];
}
...
- (void)showActionSheet:(UILongPressGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateBegan || !sender) {
        LYActionSheet *actionSheet = [[LYActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"保存图片", @"转发微博", @"赞", nil];
        [actionSheet show];
    }
}
