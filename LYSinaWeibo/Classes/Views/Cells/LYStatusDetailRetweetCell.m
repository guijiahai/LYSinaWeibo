...
NSString *const kCellIdentifier_LYStatusDetailRetweet = @"LYStatusDetailRetweetCell";

@interface LYStatusDetailRetweetCell () <TTTAttributedLabelDelegate>
...
@implementation LYStatusDetailRetweetCell
...
- (void)setCurrentStatus:(LYStatus *)currentStatus {
    ...
    _timeLabel.text = [[_currentStatus.created_at systemDate] stringDisplay_HHmm];
    
    _contentLabel.text = _currentStatus.text;
    for (NSTextCheckingResult *result in _currentStatus.items) {
        [_contentLabel addLinkToTransitInformation:@{@"text" : _currentStatus.text, @"value" : result} withRange:result.range];
    }
}
...
+ (CGFloat)cellHeightWithStatus:(LYStatus *)status {
    ...
    cellHeight += 5.0;          //间距
    cellHeight += [status.text boundingHeightWithFont:[UIFont systemFontOfSize:14] size:CGSizeMake(kScreen_Width - 60 - 20, CGFLOAT_MAX) lineSpacing:2.0];
    cellHeight += 10.0;
    return cellHeight;
}
